...
void CRC_Init(void)
{
	...
	CRC_DeInit();
		...
		#ifdef USE_CRC32
			pCRC->msgFuncCRC32 = CRC_CRC32_HWCRC;
		#endif
	#else
		#ifdef USE_CRC32
			pCRC->msgFuncCRC32 = CRC_CRC32_Table;
		#endif
	#endif
	#ifdef USE_CRC16
		pCRC->msgFuncCRC16= CRC_CRC16_Table;
	#endif
	#ifdef USE_CRC8
		pCRC->msgFuncCRC8 = CRC_CRC8_Table;
	#endif
	#ifdef USE_CHECKSUM
		pCRC->msgFuncCheckSum = CRC_CheckSum;
	#endif
}
...
UINT8_T CRC_CRC8_Table(UINT8_T *pVal, UINT32_T length)
{
	...
	for (index = 0; index < length; index++)
	{
		nTemp = nReg^ pVal[index];
		nReg =crc16Table[nTemp];
	}
	return nReg;
}
